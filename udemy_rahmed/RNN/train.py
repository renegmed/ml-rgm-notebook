...
def train(news_dataset, column_names, base_dir):
    print('Starting the training. Base dir:', base_dir)
    try:
...
#         news_dataset = pd.read_csv(os.path.join(training_path, 'newsCorpora.csv'), names=column_names, header=None, delimiter='\t')
        ...
        model.add(tf.keras.layers.Dense(8, activation='relu')) # Try 2-32 units, here and in other Dense layers
        model.add(tf.keras.layers.Dense(16, activation='relu')) # Try adding more Dense layers uncommenting this line
...
#         model.add(tf.keras.layers.LSTM(32))  # return a single vector of dimension 32
        ...
        print(model_path)
        print('Training complete. Now saving model to: ', base_dir+'/model')
        ...
        with open(os.path.join(base_dir+'/output', 'failure'), 'w') as s:
...
def _load_data(base_dir):
    column_names = ["TITLE", "URL", "PUBLISHER", "CATEGORY", "STORY", "HOSTNAME", "TIMESTAMP"]
    #dataset = np.load(os.path.join(base_dir, 'newsCorpora.csv'))
    dataset = pd.read_csv(os.path.join(base_dir, 'newsCorpora.csv'), names=column_names, header=None, delimiter='\t')
...
if __name__ == '__main__':
    ...
    dataset, column_names = _load_data(args.train)
    model = train(dataset, column_names, args.train)

    if args.current_host == args.hosts[0]:
        # save model to an S3 directory with version number '00000001'
        model.save(os.path.join(args.sm_model_dir, '000000001'), 'nlp_model.h5')
